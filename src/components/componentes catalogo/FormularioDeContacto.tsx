...
// Define la interfaz localmente para evitar conflictos
interface FormData {
    nombre: string;
    email: string;
    telefono: string;
...
// Define las claves válidas para errores
type CampoKey = 'nombre' | 'email' | 'telefono' | 'mensaje';
...
export const FormularioContacto: React.FC<FormularioContactoProps> = ({ 
    ...
    const [camposTocados, setCamposTocados] = useState<Record<CampoKey, boolean>>({
        nombre: false,
        email: false,
        telefono: false,
        mensaje: false
    ...
    const validarCampoIndividual = (name: CampoKey, value: string) => {
    ...
    const getInputClass = (campo: CampoKey, valor: string) => {
    ...
    const enviarFormulario = async (datosContacto: FormData) => {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        const todosLosCamposTocados: Record<CampoKey, boolean> = {
            nombre: true,
            email: true,
            telefono: true,
            mensaje: true
