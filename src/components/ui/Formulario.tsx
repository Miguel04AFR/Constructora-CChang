...
type Variant = 'contact' | 'consultoria';
...
export const Formulario: React.FC<Props> = ({ variant = 'contact', propiedad, formRef, onValChange, onSubmitSuccess, hideSubmit = false }) => {
  const { t } = useTranslation();
  const { isAuthenticated } = useAuth();
  const [modalLoginOpen, setModalLoginOpen] = useState(false);
  const formDataPendienteRef = useRef<any | null>(null);

  const initialData = variant === 'consultoria'
  ...
  const validarCampoIndividual = (name: string, value: string) => {
    ...
    switch (name) {
      ...
      case 'descripcion':
        if (!isContact) {
          if (!value.trim()) nuevosErrores.descripcion = t('form.errors.descriptionRequired') || 'Descripción requerida';
          else if (value.trim().length < 20) nuevosErrores.descripcion = t('form.errors.descriptionMinLength') || 'Descripción muy corta';
          else if (value.trim().length > 1000) nuevosErrores.descripcion = t('form.errors.descriptionMaxLength') || 'Descripción muy larga';
  ...
  const validarFormularioCompleto = () => {
    ...
    if (variant === 'contact') {
      ...
      if (!formData.descripcion || !formData.descripcion.trim()) nuevosErrores.descripcion = t('form.errors.descriptionRequired') || 'Descripción requerida';
      else if (formData.descripcion.trim().length < 20) nuevosErrores.descripcion = t('form.errors.descriptionMinLength') || 'Descripción muy corta';
      else if (formData.descripcion.trim().length > 1000) nuevosErrores.descripcion = t('form.errors.descriptionMaxLength') || 'Descripción muy larga';
    }
  ...
  return (
    <div className="bg-white rounded-lg shadow-sm p-6">
      <h2 className="text-xl font-bold text-[#003153] mb-4">
        {variant === 'consultoria' ? (t('consulting.form.title') || 'Formulario de consultoría') : (t('propertyDetail.contactForm.title') || 'Formulario')}
      </h2>

      {mensajeEnviado && (
        <div className='mb-4 p-3 bg-green-100 border border-green-400 text-green-700 rounded text-sm'>
          {variant === 'consultoria' ? (t('consulting.form.successMessage') || 'Enviado') : (t('propertyDetail.contactForm.successMessage') || 'Mensaje enviado')}
        </div>
      )}

      {mostrarMensajeErrores && !formularioValido && (
        <div className='mb-4 p-3 bg-red-100 border border-red-400 text-red-700 rounded text-sm'>
          {variant === 'consultoria' ? (t('consulting.form.errorMessage') || 'Corrige los errores') : (t('propertyDetail.contactForm.errorMessage') || 'Corrige los errores')}
        </div>
      )}
      ...
      <form ref={formRef} onSubmit={handleSubmit} className="space-y-4">
        ...
        {!hideSubmit && (
          <button type="submit" disabled={!formularioValido} className={`w-full py-3 px-4 rounded-md transition-colors font-medium ${formularioValido ? 'bg-[#003153] text-white hover:bg-blue-800 cursor-pointer' : 'bg-gray-300 text-gray-500 cursor-not-allowed'}`}>
            {formularioValido ? (variant === 'consultoria' ? t('consulting.form.send') : t('propertyDetail.contactForm.send')) : (variant === 'consultoria' ? t('consulting.form.completeFields') : t('propertyDetail.contactForm.completeFields'))}
          </button>
        )}

        <p className="text-xs text-gray-500 text-center">* {variant === 'consultoria' ? (t('consulting.form.requiredFields') || 'Campos requeridos') : (t('propertyDetail.contactForm.requiredFields') || 'Campos requeridos')}</p>
      </form>
